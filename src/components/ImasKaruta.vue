  ...
  <div class="container">
    ...
    <div class="filter-group">
      ...
      <div class="filter-row">
        ...
        <input type="number" v-model.number="heightInput" id="heightInput" placeholder="数字を入力" />
      ...
      <div class="filter-row">
        ...
        <input type="text" v-model="regionFilter" id="regionFilter" placeholder="例: 関東" />
      </div>
    </div>
    ...
    <div class="filter-group">
      <h2>ブランド選択</h2>
      <div v-for="(option, brand) in brandOptions" :key="brand" class="filter-row">
        <label>
    ...
    <div v-if="randomIdols.length > 0" class="playback-group">
      <button class="action-btn" @click="startManualPlayback">手動再生開始</button>
    ...
    <div v-if="randomIdols.length > 0" class="playback-group">
      ...
      <button class="action-btn" @click="startPeriodicPlayback" :disabled="isPeriodicPlaying">定期再生開始</button>
      <button class="action-btn" @click="stopPeriodicPlayback" :disabled="!isPeriodicPlaying">定期再生停止</button>
    </div>
...
function filteredByOtherCriteria() {
  return idols.value.filter(idol => {
    ...
    if (regionFilter.value !== "") {
      matchRegion = idol.region.toLowerCase().includes(regionFilter.value.toLowerCase());
    }
    return matchAge && matchBlood && matchHeight && matchBirthMonth && matchRegion;
  });
}

// 各ブランドごとの最大件数（プレースホルダー用）
function getMaxCountPlaceholder(brand) {
  const count = filteredByOtherCriteria().filter(idol => idol.brand === brand).length;
